<template>
    ...
    <div class=" ml-6 my-6 p-6 h-128 w-12/12 md:w-3/12 bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
            ...
            <div class="flex items-center mb-4" v-for="filter in filters" :key="filter" @click="()=>filterPosts(filter)">
            ...
            <div>
                ...
                <!-- Cards -->
                <div class="card-container grid grid-cols-3 gap-4 sm:grid-cols 1 md: grid-cols 2 lg:grid-cols-3" v-if="searchList.length">
                        <div v-for="recipe in searchList" :key="recipe.id" class="card">
...
export default {
    ...
    data() {
        return {
            ...
            filters:['Vegan','Primal','Whole 30','Gluten free', 'Dairy free', 'Paleolithic']
        }
    },
    methods: {
        filterPosts(catName){
    ...
    computed:{
        searchList() {
            if (this.search.trim().length) {
                return this.temp.filter((recipe) => recipe.foodName.toLowerCase().includes(this.search.trim().toLowerCase()))
            }
            return this.temp
        },
